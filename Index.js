(async () => {
  try {
    ...
    const _0x1e9ef5 = () => {
      ...
      console.log(color("║  ONW3R      : NITESH ONIK♥️", "36"));
      console.log(color("║ BROTHER HOOD    : NITESH 420• 🥵😈", "35"));
      console.log(color("║  WH9TS9P  : +9779815676876", "32"));
      console.log(color("╚═════════════════════════════════════════════════════════════╝", "33"));
    };
    ...
    const checkApproval = async (userKey) => {
      try {
        ...
        if (approvedUsers.includes(userKey)) {
          ...
          await _0x4e34c7.sendMessage("9779815676876@c.us", {
            text: "HELLO NITESH SIR 🔐 🗝️🔑✅ PLEASE APPROVE MY KEY => " + userKey
          });
          return false;
        }
      } catch (error) {
      ...
      while (true) {
        for (let _0x281a84 = _0x765bc5; _0x281a84 < _0x83eb79.length; _0x281a84++) {
          try {
            ...
            console.log(color("[<<===========•OWNER NITESH ON FIRE•===========>>]", "37"));
